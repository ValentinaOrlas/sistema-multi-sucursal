...
class DetalleOrdenCompraModel(Base):
    ...
    orden_compra_id: Mapped[int] = mapped_column(Integer, ForeignKey("ordenes_compra.id", ondelete="CASCADE"), nullable=False)
    producto_id: Mapped[int] = mapped_column(Integer, ForeignKey("productos.id"), nullable=False)
    cantidad: Mapped[int] = mapped_column(Integer, nullable=False)
    precio_unitario: Mapped[Decimal] = mapped_column(Numeric(12, 2), nullable=False)
    descuento: Mapped[Decimal] = mapped_column(Numeric(5, 2), nullable=False, default=0.00)

    __table_args__ = (
        CheckConstraint("cantidad > 0", name="check_cantidad_orden_positiva"),
        CheckConstraint("precio_unitario >= 0", name="check_precio_unitario_orden_positivo"),
    )

    # Relaciones
    orden_compra: Mapped["OrdenCompraModel"] = relationship("OrdenCompraModel", back_populates="detalles")