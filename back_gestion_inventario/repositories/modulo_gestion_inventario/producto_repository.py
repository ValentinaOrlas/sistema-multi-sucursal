...
from exceptions.modulo_gestion_inventario.producto_exceptions import SkuDuplicado
...
class ProductoRepository:
    ...
    def bloquear_sucursal(self, sucursal_id: int) -> dict | None:
        ...
        """
        # PostgreSQL mantiene el bloqueo hasta el commit o rollback. SQLite,
        # utilizado en las pruebas rápidas, no admite esta cláusula.
    ...
    def insertar_producto(self, producto: dict) -> dict:
        consulta = text("""
            INSERT INTO productos (
                sku, nombre, categoria_id, unidad_medida, stock_minimo_global
            )
            VALUES (
                :sku, :nombre, :categoria_id, :unidad_medida, 0
            )
            RETURNING id, sku, nombre, categoria_id, unidad_medida
        """)
        try:
